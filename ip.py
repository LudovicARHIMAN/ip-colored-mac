...
def remove_chars(input_string, start_index, end_index):
    result = input_string[:start_index] + input_string[end_index+1:]
    return result
...
def do_addr_show(argv, af):
    ...
    for r in res.split("\n"):
        if re.match(r"^\w", r):
            ...
            address_count = 0
            buff += f"{colorize(ifname,COLOR_CYAN)}"+(r.rstrip() + "\n").replace(ifname,"")
        elif re.match(r"^\W+inet" + SIX + ".+", r):
            address_count += 1
            if re.match(r"^\W+inet .+", r):
                cols = r.split()
                ip_addr = cols[1]
                buff += f"{colorize(r.rstrip(), COLOR_GREEN)} {ifname}\n"
            else:
                buff += r.rstrip() + "\n"
        elif re.match(r"^\W+inet6" + SIX + ".+", r):
            address_count += 1
            if re.match(r"^\W+inet6 .+", r):
                cols = r.split()
                ipv6_addr = cols[1]
                buff += "inet" + f"{colorize(r.rstrip(), COLOR_CYAN)} {ifname}\n"
            else:
                buff += r.rstrip() + "\n"
        elif re.match(r"^\W+ether.+", r):
            cols = r.split()
            buff +=  f"{colorize(r.rstrip(), COLOR_MAGENTA)} {ifname}\n"
    if address_count > 0:
        output += buff
    print(output.rstrip())
    return True
